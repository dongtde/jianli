...
const quoteLabel = computed(() => props.typedQuote || props.heroQuotes[0] || '')
</script>

<template>
  <section id="home" class="scene-section hero-section is-visible" :class="{ active }" aria-labelledby="home-title">
    <div class="hero-landing">
      ...
      <nav class="hero-shortcuts" aria-label="首页快速入口">
        <button type="button" title="查看代表项目" @click="$emit('navigate', 'projects')">
          <Map :size="20" />
          <span>项目</span>
        </button>
        <button type="button" title="查看职业经历" @click="$emit('navigate', 'route')">
